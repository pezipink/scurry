...
(scurry
 ...
 (def-λ (var=? x1 x2) (eq (nth x1 0) (nth x2 0)))
 (def-λ (assp f l)
   (if (eq l #f)
       #f
       (begin
...
(def zz (list (list (var "x") (var "y")) (list (var "z") "e")))

;; (dbgl " A " (walk (var "x") zz))
;; (dbgl " B " (walk (var "y") zz))
...
;; (dbgl "w2 " (walk (var "x") zz))

;; (dbgl "w2 " (walk (var "x") (head zz)))

;; (dbgl "w8 " (unify #t (var "z")  zz))
...
(def-λ (=== u v)
  (λ (s/c)
    ...
    ;; (dbgl "tail sc is " (head (tail s/c)))
    (let ([s (unify u v (head s/c))])
      (if s (units (list s (list (head (tail s/c))))) (list)))))

(def s/c-empty (list (list) 0))
...
(def lag (fives-and-sixes s/c-empty))
